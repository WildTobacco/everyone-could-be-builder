...
def ke_hoach_ds_mbnt_nam() -> pd.Series:
    """Annual (month='total') DS MBNT plan by nhom_phu_trach, + TOTAL."""
...
@_df_cache()
def ke_hoach_ds_mbnt_thang(month_value: str) -> pd.Series:
    """Same source/shape as ke_hoach_ds_mbnt_nam, but for a single month (e.g. "8" for
    ...
    see ke_hoach_ds_mbnt_thang_only for that."""
    df = ke_hoach_theo_ptkd()
...
@_df_cache()
def ke_hoach_ds_mbnt_thang_only(month_value: str) -> pd.Series:
    """Non-cumulative: just the selected month's own DS MBNT plan portion, i.e. this
    ...
    (single-day) actuals like DS_Current, instead of ke_hoach_ds_mbnt_thang's YTD figure."""
    month_num = int(month_value)
    current = ke_hoach_ds_mbnt_thang(month_value)
    if month_num == 1:
        return current
    return current - ke_hoach_ds_mbnt_thang(str(month_num - 1))


if __name__ == "__main__":
    print(ke_hoach_ds_mbnt_nam())
    print(ke_hoach_ds_mbnt_thang("8"))
    print(ke_hoach_ds_mbnt_thang_only("8"))
