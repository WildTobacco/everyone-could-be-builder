...
import pandas as pd
from data_connect.excel_connect import khkdcn_mbnt_nhom_phu_trach_2026, listbds
...
def _kh_filtered(date_str: str) -> pd.DataFrame:
    ...
    df = khkdcn_mbnt_nhom_phu_trach_2026()
    return df[df["Month"] <= selected_month]


@_df_cache()
def kh_mbnt_nhom_dia_ban_2026(date_str: str) -> pd.Series:
    """SUM(KHKDCN_mbnt_nhom_phu_trach_2026[KHKD KDNT]) WHERE Month <= SelectedMonth,
    grouped by Nhóm phụ trách + TOTAL. No SelectedBDS/dim_bdscif membership filter
    (that table isn't available outside Power BI)."""
    df = _kh_filtered(date_str)
    by_group = df.groupby("Nhóm phụ trách")["KHKD KDNT"].sum()
    total = by_group.sum()
...
@_df_cache()
def kh_mbnt_by_nhom_diaban(date_str: str) -> pd.Series:
    ...
    result = df.groupby(["Nhóm phụ trách", "diaban"])["KHKD KDNT"].sum()
...
if __name__ == "__main__":
    ...
    d = latest_date()
    print(f"date: {d}")
    print(kh_mbnt_nhom_dia_ban_2026(d))
    print("\nBy dia_ban:")
    print(kh_mbnt_by_nhom_diaban(d).to_string())
