...
import pandas as pd
from data_connect.excel_connect import khkdcn_tdps_nhom_phu_trach_2026, listbds
...
def _kh_filtered(date_str: str) -> pd.DataFrame:
    ...
    df = khkdcn_tdps_nhom_phu_trach_2026()
    return df[df["Month"] <= selected_month]


@_df_cache()
def kh_tdps_nhom_dia_ban_2026(date_str: str) -> pd.Series:
    """Same shape as kh_mbnt_nhom_dia_ban_2026, sourced from KHKD CN 2026.xlsx's "KHKD TDPS"
    columns instead of "KHKD KDNT"."""
    df = _kh_filtered(date_str)
    by_group = df.groupby("Nhóm phụ trách")["KHKD TDPS"].sum()
    total = by_group.sum()
...
@_df_cache()
def kh_tdps_by_nhom_diaban(date_str: str) -> pd.Series:
    ...
    result = df.groupby(["Nhóm phụ trách", "diaban"])["KHKD TDPS"].sum()
...
if __name__ == "__main__":
    ...
    d = latest_date()
    print(f"date: {d}")
    print(kh_tdps_nhom_dia_ban_2026(d))
    print("\nBy dia_ban:")
    print(kh_tdps_by_nhom_diaban(d).to_string())
