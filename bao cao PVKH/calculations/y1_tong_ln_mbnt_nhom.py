...
@_df_cache()
def compute_y1_tong_ln_mbnt_nhom(date_str: str) -> pd.Series:
    """date_str is already the Y-1 (same-day-last-year) target date — caller shifts, this func does not.
    Full + Partial x ProrationRatio approximation, same pattern as tong_ln_nhom / y1_tong_ds_mbnt_nhom."""
...
if __name__ == "__main__":
    ...
    print(compute_y1_tong_ln_mbnt_nhom(ref_2025).apply(lambda x: f"{x:,.2f}"))
