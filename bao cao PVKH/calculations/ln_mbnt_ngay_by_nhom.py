"""Python port of LN_MBNT_PTKD1_update / LN_MBNT_PTKD2_update / LN_MBNT_VPV_update /
LN_MBNT_HT_update, from a different, pre-existing dashboard's DAX (not this project's
...
columns instead: pvkh_MBNT_Nhom_phu_trach_ngay[sum_LN_MBNT_ngay_bao_cao] ->
sum_ln_mbnt_ngay_bao_cao, [Nhom_phu_trach] -> nhom_phu_trach (verified identical values,
just different casing).

    CALCULATE(SUM(pvkh_MBNT_Nhom_phu_trach_ngay[sum_LN_MBNT_ngay_bao_cao]), [Nhom_phu_trach] = "<X>")
"""
...
@_df_cache()
def ln_mbnt_ngay_by_nhom(date_str: str):
    ...
    PTKD1+PTKD2+VPV like tong_ln_mbnt_ytd_by_nhom does for the YTD/lũy kế measure.
    .max() (not .sum()) matches the defensive groupby already used for this same table
    in tong_ln_mbnt_ytd.py, in case of duplicate rows for a (nhóm, ngày) pair."""
    df = pvkh_mbnt_nhom_phu_trach(date_str)
    by_group = df.groupby("nhom_phu_trach")["sum_ln_mbnt_ngay_bao_cao"].max()
    return by_group.reindex(GROUPS + ["TOTAL"], fill_value=0.0)


if __name__ == "__main__":
    print(ln_mbnt_ngay_by_nhom(latest_date()))
