...
### 2026-09-06 14:20 ICT — Claude Sonnet 5 (session 01NPwi3M)
<u>**Changed:**</u> the entry-format template and `.github/scripts/update_exchange_log.py` again, plus this file's own 14:10 entry below.

<u>**Why:**</u> the 14:10 entry switched the three labels from `**bold**` to `<u>underline</u>`, but the user then clarified they want both underlined and bold together. Changed the template/script to `<u>**Label:**</u>` and rewrote the 14:10 entry to match, so the log doesn't carry a stale example of the wrong format.

<u>**Open / for next agent:**</u> use `<u>**Label:**</u>` for the three labels in any entry you write by hand from now on; entries before 14:10 are left as plain `**bold**` and not being rewritten.
